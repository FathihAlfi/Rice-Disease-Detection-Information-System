...
<div class="max-w-6xl mx-auto p-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="bg-white border rounded-lg p-6 shadow-sm">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Daftar Jenis Tanaman</h2>
            ...
            @if($jenisTanaman->isEmpty())
                <p class="text-gray-500">Belum ada data jenis tanaman.</p>
            @else
                ...
                <ul class="divide-y">
                    @foreach ($jenisTanaman as $jenis)
                        <li class="p-4 flex justify-between items-center hover:bg-gray-50 transition">
                            ...
                            <form id="delete-form-{{ $jenis->jenis_id }}" action="{{ route('jenis.destroy', $jenis->jenis_id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="button" data-form-id="delete-form-{{ $jenis->jenis_id }}" class="delete-button bg-red-600 hover:bg-red-700 text-white px-4 py-1 rounded text-sm shadow">
                                    Hapus
                                </button>
                            </form>
                        </li>
        ...
        <div class="bg-white border rounded-lg p-6 shadow-sm h-fit">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Tambah Jenis Tanaman</h2>

            <form method="POST" action="{{ route('jenis.store') }}" class="space-y-4">
                @csrf
                <div>

...
<div class="max-w-6xl mx-auto p-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="bg-white border rounded-lg p-6 shadow-sm">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Daftar Metode Diagnosa</h2>
            ...
            @if($metodes->isEmpty())
                <p class="text-gray-500">Belum ada data metode.</p>
            @else
                ...
                <ul class="divide-y">
                    @foreach ($metodes as $metode)
                        <li class="p-4 flex justify-between items-center hover:bg-gray-50 transition">
                            ...
                            <form id="delete-form-{{ $metode->metode_id }}" action="{{ route('metode.destroy', $metode->metode_id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="button" data-form-id="delete-form-{{ $metode->metode_id }}" class="delete-button bg-red-600 hover:bg-red-700 text-white px-4 py-1 rounded text-sm shadow">
                                    Hapus
                                </button>
                            </form>
                        </li>
        ...
        <div class="bg-white border rounded-lg p-6 shadow-sm h-fit">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Tambah Metode Diagnosa</h2>

            <form method="POST" action="{{ route('metode.store') }}" class="space-y-4">
                @csrf
                <div>
                    <label for="nama_metode" class="block font-medium text-gray-700">Nama Metode</label>
                    <input
                        type="text"
                        id="nama_metode"
                        name="nama_metode"
                        ...
                        placeholder="Contoh: Certainty Factor"
                    >
                    @error('nama_metode')
